...
DECLARE
  ...
  v_receipt_type TEXT;
BEGIN
  SELECT p.client_id::text, p.car_id::text, p.policy_type_parent
  INTO v_client_id, v_car_id, v_receipt_type
  FROM policies p
  WHERE p.id = NEW.policy_id::uuid;

  SELECT full_name INTO v_client_name
  FROM clients WHERE id = v_client_id::uuid;

  IF v_car_id IS NOT NULL THEN
    SELECT car_number INTO v_car_number
    FROM cars WHERE id = v_car_id::uuid;
  END IF;
